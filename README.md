...
The chick-heart benchmark is treated separately as a period-doubling / alternans problem, so it uses lag-2 and alternans-aware feature augmentation, reports a `Lag2-CSD` baseline, and adds a `Kalman-LSTM-Aux` model that learns lag-2 / alternans auxiliaries.
